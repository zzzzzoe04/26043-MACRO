...
#define LRAS_PER_MUX 5
#define TOTAL_LRAS 10
...
// Timing settings, each phase should be 30s (30000ms) 
...
const int precond_phase = 30000;   // pre condition phase (3000 min)
const int active_phase = 30000;   // stimulation phase (2000 min)
const int rest_phase = 30000;      // rest phase    (5000 min) 
...
/* Select specific mux + channel */
void tcaSelect(uint8_t muxAddr, uint8_t channel) {
  if (channel > 7) return;

  disableAllMux(); // ensure only one path is active
...
drv.go();
delay(1); // if I2C flooding occurs, change delay to 5
}
...
void setup() {
    ...
    for (uint8_t i = 0; i < TOTAL_LRAS; i++) {
        ...
        muxAddr = MUX2_ADDR;
        channel = i - LRAS_PER_MUX;
        }

        Serial.print("Initializing LRA ");
        Serial.print(i);
        ...
        delay(5); // optional but good practice to keep
        initDRV();
        delay(10);
    }
...
void loop() {
        ...
        if (data_flag && counter < 6000) { // if we are actively collecting data
            ...
            if (now >= next_sample) { // it's time to collect a sample
                ...
                if (!LRA_ON && now >= activation_time && now < activation_end){ // if the LRA is off and its time to activate (and less than activation end bc we dont want to turn it back on after we have already turned it off for that trial)
                    ...
                    if (active_digit < LRAS_PER_MUX) {
                    muxAddr = MUX1_ADDR;
                    channel = active_digit;
                    } else {
                    muxAddr = MUX2_ADDR;
                    channel = active_digit - LRAS_PER_MUX;
                    }
                    tcaSelect(muxAddr, channel);
                    // delay(5); if there are bugs, try adding this delay in (wasn't necessary for 5 LRAs but may be for 10)
    ...
    if (!assessment_mode) { // if you werent told to run in assessment mode, run in training mode
        ...
        if (trial_done_flag) { // trial done flag
